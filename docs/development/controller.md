...
The `controllers` directory is the firmware workspace for all flow-controller hardware.
...
in `boards/`. The shared [`main.c`](../../controllers/main.c) is the controller entry point on
...
Execution uses the strict [PLC scan cycle](../architecture/plc-scan-cycle.md): Read
...
[controller setup guide](controller-setup.md) before building.

See [controller features](../reference/controller-features.md) for the implemented firmware capabilities and
[portable flow runtime architecture](../architecture/portable-flow-runtime.md) for the current execution design.
...
network management, MQTT, RS485 framing, and the flow runtime must not
include vendor board or SDK headers.

## Workspace and board selection

Run the commands in this guide from `controllers` unless stated otherwise. Open
that directory directly in VS Code. Run **Tasks: Run Task**
...
The repository's [`.clang-format`](../../controllers/.clang-format) configuration uses the
...
Implemented capabilities are summarized in [controller features](../reference/controller-features.md), the
execution architecture is in [portable flow runtime architecture](../architecture/portable-flow-runtime.md), and
the normative bespoke wire contract is in [controller protocol v1](../reference/controller-protocol-v1.md).
