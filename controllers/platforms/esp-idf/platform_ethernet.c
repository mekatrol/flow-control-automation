...
static void enqueue_platform_event(const ethernet_platform_event_t *event)
{
    ...
    {
        xQueueSend(ethernet_event_queue, event, 0);
    }
}
...
static bool is_dns_ready(void)
{
    ...
    if (esp_netif_get_dns_info(ethernet_network_interface, ESP_NETIF_DNS_MAIN, &dns) != ESP_OK)
    {
        return false;
    }
...
static void handle_ip_event(void * /* context */, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    ...
    if (event_id == IP_EVENT_ETH_GOT_IP)
    {
        ...
        if (got_ip->esp_netif != ethernet_network_interface)
        {
            return;
        }
    ...
    else if (event_id == IP_EVENT_GOT_IP6)
    {
        ...
        if (got_ip6->esp_netif != ethernet_network_interface)
        {
            return;
        }
...
bool platform_ethernet_initialize(const ethernet_link_config_t *config)
{
    ...
    if (!is_gpio_interrupt_service_ready())
    {
        return false;
    }

    ethernet_event_queue = xQueueCreate(ETHERNET_EVENT_QUEUE_DEPTH, sizeof(ethernet_platform_event_t));
    ...
    if (spi_bus_initialize(ETHERNET_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO) != ESP_OK)
    {
        return false;
    }
    ...
    eth_w5500_config_t w5500_config = ETH_W5500_DEFAULT_CONFIG(ETHERNET_SPI_HOST, &device_config);
    ...
    if (ethernet_network_interface == NULL || esp_netif_set_hostname(ethernet_network_interface, config->hostname) != ESP_OK)
    {
        return false;
    }
...
{
    return ethernet_event_queue != NULL && xQueueReceive(ethernet_event_queue, event, 0) == pdTRUE;
}
