...
enum { WIFI_EVENT_QUEUE_DEPTH = 16 };
...
static bool is_dns_ready(void)
{
    ...
    if (esp_netif_get_dns_info(wifi_network_interface, ESP_NETIF_DNS_MAIN,
                               &dns) != ESP_OK)
        return false;
                              ...
                              int32_t event_id, void *event_data)
{
    ...
    case WIFI_EVENT_STA_DISCONNECTED: {
        ...
        event.type = is_authentication_failure(disconnected->reason)
                            ...
                            int32_t event_id, void *event_data)
{
    ...
    if (event_id == IP_EVENT_STA_GOT_IP) {
        ...
        if (got_ip->esp_netif != wifi_network_interface) return;
        ...
        if (got_ip6->esp_netif != wifi_network_interface) return;
        event.type = WIFI_PLATFORM_EVENT_ADDRESS_READY;
        event.dns_ready = is_dns_ready();
...
void platform_wifi_get_config(wifi_link_config_t *config)
{
    *config = (wifi_link_config_t) {
        ...
        .power_save = CONFIG_CONTROLLER_WIFI_POWER_SAVE
...
{
    if (!initialize_persistence()) return false;
    esp_err_t result = esp_netif_init();
    if (result != ESP_OK && result != ESP_ERR_INVALID_STATE) return false;
    result = esp_event_loop_create_default();
    if (result != ESP_OK && result != ESP_ERR_INVALID_STATE) return false;
    wifi_event_queue = xQueueCreate(WIFI_EVENT_QUEUE_DEPTH,
                                    sizeof(wifi_platform_event_t));
    if (wifi_event_queue == NULL) return false;
    wifi_network_interface = esp_netif_create_default_wifi_sta();
    if (wifi_network_interface == NULL) return false;
    ...
    (void)memcpy(station.sta.password, config->password,
                 strlen(config->password));
    /* Match ESP-IDF station guidance: protected credentials require WPA2 or better. */
    station.sta.threshold.authmode = config->password[0] != '\0'
                                         ? WIFI_AUTH_WPA2_PSK
                                         : WIFI_AUTH_OPEN;
    ...
    const wifi_ps_type_t power_save =
        config->power_save == WIFI_POWER_SAVE_MINIMUM_MODEM
            ? WIFI_PS_MIN_MODEM
            : WIFI_PS_NONE;
...
{
    if (is_driver_started) return esp_wifi_connect() == ESP_OK;
...
{
    if (!is_driver_started) return false;
...
{
    if (is_driver_started) (void)esp_wifi_stop();
}
...
{
    return wifi_event_queue != NULL &&
           xQueueReceive(wifi_event_queue, event, 0) == pdTRUE;
}
