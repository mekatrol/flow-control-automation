...
static const char FORMAT_NETWORK_CONFIGURATION[] =
    "wifi=runtime_disabled wifi_credentials=%s ethernet=%s";
...
{
    const bool is_wifi_configured = CONFIG_CONTROLLER_WIFI_SSID[0] != '\0' &&
                                    CONFIG_CONTROLLER_WIFI_PASSWORD[0] != '\0';
    ...
    (void)snprintf(output, output_size, FORMAT_NETWORK_CONFIGURATION,
                   is_wifi_configured ? CREDENTIALS_CONFIGURED
                                      : CREDENTIALS_NOT_CONFIGURED,
                   CONFIG_CONTROLLER_ETHERNET_ENABLED ? FEATURE_ENABLED
                                                      : FEATURE_DISABLED);
}
