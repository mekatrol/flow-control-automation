...
#error "FLOW_IL_V1_FIXTURE_DIRECTORY must identify the shared fixture directory"
#endif
...
{
    char path[512];
    const int path_length = snprintf(path, sizeof(path), "%s/%s/artifact.bin", FLOW_IL_V1_FIXTURE_DIRECTORY, fixture_id);
...
/* What: Independently decodes bounded Flow IL metadata. Why: C and .NET must agree before VM execution. How:
...
{
    if (bytes == NULL || metadata == NULL || size < TEST_ENVELOPE_LENGTH || memcmp(bytes, "FIL1", 4) != 0 ||
        get_u16(&bytes[4]) != 1U || get_u16(&bytes[6]) != TEST_ENVELOPE_LENGTH)
    {
    ...
    for (uint16_t index = 0; index < section_count; index++)
    {
        ...
        if (version != 1U || offset != expected_offset || length > size - offset)
        {
            return TEST_FLOW_IL_MALFORMED;
        }
