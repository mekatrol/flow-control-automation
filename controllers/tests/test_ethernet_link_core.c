...
{
    assert(ethernet_link_get_network_event_type(
               ETHERNET_PLATFORM_EVENT_DRIVER_STARTED) == NETWORK_EVENT_STARTED);
    assert(ethernet_link_get_network_event_type(
               ETHERNET_PLATFORM_EVENT_LINK_UP) == NETWORK_EVENT_CONNECTING);
    assert(ethernet_link_get_network_event_type(
               ETHERNET_PLATFORM_EVENT_ADDRESS_READY) == NETWORK_EVENT_ONLINE);
    assert(ethernet_link_get_network_event_type(
    ...
    assert(ethernet_link_get_network_event_type(
           ...
           NETWORK_EVENT_CONNECTION_LOST);
    assert(ethernet_link_get_network_event_type(
               ETHERNET_PLATFORM_EVENT_DRIVER_FAILED) == NETWORK_EVENT_FAILED);
    assert(ethernet_link_get_network_event_type(
               ETHERNET_PLATFORM_EVENT_STOPPED) == NETWORK_EVENT_STOPPED);
}
