...
static network_manager_t get_test_manager(fixture_t *fixture, bool wifi, bool ethernet)
{
                          ...
                          bool dns, const char *reason)
{
    const network_event_t value = {
        ...
        .type           = type,
        .sequence       = sequence,
        .interface_name = id == NETWORK_LINK_WIFI ? WIFI_INTERFACE : ETHERNET_INTERFACE,
...
static void test_transitions_and_stale_events(void)
{
    ...
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_STARTED, FIRST_SEQUENCE, false, REASON_DRIVER_READY);
    ...
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_ONLINE, SECOND_SEQUENCE, true, REASON_ADDRESS_READY);
    network_manager_process(&manager, SECOND_PROCESS_TIME_MS);
    assert(manager.links[NETWORK_LINK_WIFI].state == NETWORK_LINK_ONLINE);
    assert(strcmp(manager.links[NETWORK_LINK_WIFI].ipv4_address, TEST_IPV4_ADDRESS) == 0);
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_FAILED, SECOND_SEQUENCE, false, REASON_DUPLICATE);
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_FAILED, FIRST_SEQUENCE, false, REASON_STALE);
    network_manager_process(&manager, THIRD_PROCESS_TIME_MS);
    assert(manager.links[NETWORK_LINK_WIFI].state == NETWORK_LINK_ONLINE);
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_DEGRADED, THIRD_SEQUENCE, false, REASON_DNS_LOST);
    network_manager_process(&manager, FOURTH_PROCESS_TIME_MS);
    assert(manager.links[NETWORK_LINK_WIFI].state == NETWORK_LINK_DEGRADED);
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_CONNECTION_LOST, FOURTH_SEQUENCE, false, REASON_LINK_LOST);
...
static void test_backoff_jitter_stability_and_shutdown(void)
{
    ...
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_FAILED, FIRST_SEQUENCE, false, REASON_FAILED);
    ...
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_FAILED, SECOND_SEQUENCE, false, REASON_FAILED_AGAIN);
    ...
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_ONLINE, THIRD_SEQUENCE, true, REASON_RECOVERED);
...
static void test_link_selection_and_independence(void)
{
    ...
    assert(!network_manager_get_selected_link(&manager, NETWORK_ROUTE_AUTOMATIC, true, &selected));
    enqueue_event(&manager, NETWORK_LINK_WIFI, NETWORK_EVENT_ONLINE, FIRST_SEQUENCE, true, REASON_WIFI_UP);
    network_manager_process(&manager, FIRST_PROCESS_TIME_MS);
    assert(network_manager_get_selected_link(&manager, NETWORK_ROUTE_AUTOMATIC, true, &selected));
    assert(selected == NETWORK_LINK_WIFI);
    enqueue_event(&manager, NETWORK_LINK_ETHERNET, NETWORK_EVENT_ONLINE, FIRST_SEQUENCE, true, REASON_ETHERNET_UP);
    network_manager_process(&manager, SECOND_PROCESS_TIME_MS);
    assert(network_manager_get_selected_link(&manager, NETWORK_ROUTE_AUTOMATIC, true, &selected));
    assert(selected == NETWORK_LINK_ETHERNET);
    assert(network_manager_get_selected_link(&manager, NETWORK_ROUTE_WIFI, true, &selected));
    assert(selected == NETWORK_LINK_WIFI);
    enqueue_event(&manager, NETWORK_LINK_ETHERNET, NETWORK_EVENT_FAILED, SECOND_SEQUENCE, false, REASON_CABLE_LOST);
    network_manager_process(&manager, THIRD_PROCESS_TIME_MS);
    assert(manager.links[NETWORK_LINK_WIFI].state == NETWORK_LINK_ONLINE);
    assert(network_manager_get_selected_link(&manager, NETWORK_ROUTE_AUTOMATIC, true, &selected));
    assert(selected == NETWORK_LINK_WIFI);
    assert(!network_manager_get_selected_link(&manager, NETWORK_ROUTE_ETHERNET, true, &selected));
}
...
static void test_queue_bounds_and_enable_disable(void)
{
    ...
    network_manager_set_enabled(&manager, NETWORK_LINK_WIFI, true, FIRST_PROCESS_TIME_MS);
    assert(manager.links[NETWORK_LINK_WIFI].state == NETWORK_LINK_STARTING);
...
static void test_event_queue_owns_callback_data(void)
{
    ...
    reason[0]  = 'X';
    network_manager_process(&manager, FIRST_PROCESS_TIME_MS);
    assert(strcmp(manager.links[NETWORK_LINK_WIFI].ipv4_address, OWNED_IPV4_ADDRESS) == 0);
    assert(strcmp(manager.links[NETWORK_LINK_WIFI].last_transition_reason, REASON_DHCP_READY) == 0);
}
