...
static void test_shared_value_and_ownership(void)
{
    ...
    assert(flow_virtual_points_activate(&store, TEST_INSTANCE, "writer-c", &writer, 1U) == FLOW_VIRTUAL_POINT_WRITER_CONFLICT);
    const flow_virtual_point_command_t command = {.key = "temperature", .type = FLOW_VIRTUAL_POINT_ANALOG, .analog_value = 21.5};
...
{
    flow_virtual_point_store_t store                 = get_store();
    flow_virtual_point_declaration_t declarations[2] = {
        get_temperature(true),
        {.key = "enabled", .type = FLOW_VIRTUAL_POINT_DIGITAL, .persistence = FLOW_VIRTUAL_POINT_VOLATILE, .is_writer = true}};
    assert(flow_virtual_points_activate(&store, TEST_INSTANCE, "writer", declarations, 2U) == FLOW_VIRTUAL_POINT_OK);
    flow_virtual_point_command_t commands[2] = {{.key = "temperature", .type = FLOW_VIRTUAL_POINT_ANALOG, .analog_value = 22.0},
                                                {.key = "enabled", .type = FLOW_VIRTUAL_POINT_ANALOG, .analog_value = 1.0}};
...
static void test_typed_retained_round_trip(void)
{
    ...
    const flow_virtual_point_command_t command = {.key = "temperature", .type = FLOW_VIRTUAL_POINT_ANALOG, .analog_value = 19.25};
    ...
    image[8U + (2U * FLOW_VIRTUAL_POINT_ID_CAPACITY)] = FLOW_VIRTUAL_POINT_DIGITAL;
    assert(flow_virtual_points_restore_retained(&restored, image, image_size) == FLOW_VIRTUAL_POINT_RETAINED_INCOMPATIBLE);
}
