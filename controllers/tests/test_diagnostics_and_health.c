...
static const char EXPECTED_EVENT[] =
...
static void test_event_formatting(void)
{
    ...
    assert(diagnostic_format_event(output, sizeof(output), DIAGNOSTIC_WARNING,
                                   COMPONENT_WIFI, EVENT_ASSOCIATION_FAILED,
...
static void test_rate_limiting(void)
{
    ...
    assert(is_diagnostic_event_allowed(&limiter, RATE_WINDOW_STARTED_MS,
                                       RATE_WINDOW_MS, RATE_MAXIMUM_EVENTS,
                                       &suppressed));
    assert(suppressed == 0);
    assert(is_diagnostic_event_allowed(&limiter, RATE_SECOND_EVENT_MS,
                                       RATE_WINDOW_MS, RATE_MAXIMUM_EVENTS,
                                       &suppressed));
    assert(!is_diagnostic_event_allowed(&limiter, RATE_FIRST_REJECTED_MS,
                                        RATE_WINDOW_MS, RATE_MAXIMUM_EVENTS,
                                        &suppressed));
    assert(!is_diagnostic_event_allowed(&limiter, RATE_SECOND_REJECTED_MS,
                                        RATE_WINDOW_MS, RATE_MAXIMUM_EVENTS,
                                        &suppressed));
    assert(is_diagnostic_event_allowed(&limiter, RATE_NEXT_WINDOW_MS,
                                       RATE_WINDOW_MS, RATE_MAXIMUM_EVENTS,
                                       &suppressed));
