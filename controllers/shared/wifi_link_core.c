...
bool is_wifi_link_config_valid(const wifi_link_config_t *config)
{
    if (config == NULL || config->ssid == NULL || config->password == NULL ||
        ...
        return false;
    return strlen(config->ssid) <= WIFI_SSID_MAX_LENGTH &&
           strlen(config->password) <= WIFI_PASSWORD_MAX_LENGTH &&
           strlen(config->hostname) <= WIFI_HOSTNAME_MAX_LENGTH &&
           config->hostname[0] != '\0';
}
    ...
    wifi_platform_event_type_t platform_type)
{
    ...
    case WIFI_PLATFORM_EVENT_DRIVER_STARTED: return NETWORK_EVENT_STARTED;
    case WIFI_PLATFORM_EVENT_ASSOCIATING:
    case WIFI_PLATFORM_EVENT_ASSOCIATED: return NETWORK_EVENT_CONNECTING;
    case WIFI_PLATFORM_EVENT_ADDRESS_READY: return NETWORK_EVENT_ONLINE;
    case WIFI_PLATFORM_EVENT_ADDRESS_LOST: return NETWORK_EVENT_CONNECTION_LOST;
    case WIFI_PLATFORM_EVENT_AUTHENTICATION_FAILED:
    case WIFI_PLATFORM_EVENT_ASSOCIATION_FAILED:
    case WIFI_PLATFORM_EVENT_DRIVER_FAILED: return NETWORK_EVENT_FAILED;
    case WIFI_PLATFORM_EVENT_STOPPED: return NETWORK_EVENT_STOPPED;
