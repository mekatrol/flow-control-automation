...
enum { PERCENT_SCALE = 100U, EXPONENTIAL_BACKOFF_MULTIPLIER = 2U };
...
static void set_link_state(network_link_snapshot_t *link,
...
static uint32_t get_backoff_delay(network_manager_t *manager,
                          ...
                          uint64_t now_ms)
{
    ...
    for (network_link_id_t id = NETWORK_LINK_WIFI; id < NETWORK_LINK_COUNT; id++) {
        ...
        set_link_state(&manager->links[id],
                   configs[id].enabled ? NETWORK_LINK_STARTING
                                       : NETWORK_LINK_DISABLED,
                   now_ms, configs[id].enabled ? REASON_ENABLED
...
{
    if (manager == NULL || event == NULL || !is_valid_link(event->link_id)) return false;
...
void network_manager_set_enabled(network_manager_t *manager,
                                 ...
                                 uint64_t now_ms)
{
    if (!is_valid_link(link_id) || manager->config[link_id].enabled == enabled) return;
    manager->config[link_id].enabled = enabled;
    if (enabled) {
        begin_start(manager, link_id, now_ms, REASON_MANUALLY_ENABLED);
    } else {
        set_link_state(&manager->links[link_id], NETWORK_LINK_DISABLED, now_ms,
                       REASON_MANUALLY_DISABLED);
...
void network_manager_shutdown(network_manager_t *manager, uint64_t now_ms)
{
    for (network_link_id_t id = NETWORK_LINK_WIFI; id < NETWORK_LINK_COUNT; id++) {
        ...
        set_link_state(&manager->links[id], NETWORK_LINK_STOPPED, now_ms,
                       REASON_SHUTDOWN);
    }
...
static bool is_link_eligible(const network_manager_t *manager,
                             network_link_id_t id, bool require_dns)
{
                                       ...
                                       network_link_id_t *selected_link)
{
    ...
    if (policy != NETWORK_ROUTE_AUTOMATIC) {
        ...
        if (!is_link_eligible(manager, id, require_dns)) return false;
        *selected_link = id;
        return true;
    }
...
{
    if (link_id == NETWORK_LINK_WIFI) return LINK_NAME_WIFI;
    if (link_id == NETWORK_LINK_ETHERNET) return LINK_NAME_ETHERNET;
...
const char *network_get_link_state_name(network_link_state_t state)
{
    static const char *const names[] = {
        STATE_DISABLED, STATE_STARTING, STATE_CONNECTING, STATE_ONLINE,
