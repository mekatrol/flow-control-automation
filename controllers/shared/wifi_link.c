...
static const char MESSAGE_CONFIG_INVALID[] =
    "station disabled because configuration exceeds supported limits";
static const char MESSAGE_PLATFORM_INIT_FAILED[] =
    "station disabled because platform initialization failed";
static const char MESSAGE_CONNECTION_ATTEMPT_FAILED[] =
    "platform rejected the non-blocking connection request";
static const char FORMAT_EVENT_STATE[] = "state_change=%s";
static const char FORMAT_ADDRESS_READY[] =
    "ipv4=%s ipv6=%s dns_ready=%u";
static const char FORMAT_FAILURE_STATE[] =
    "state_change=%s reason_code=%u rssi_dbm=%d";
...
static const char *get_event_reason(wifi_platform_event_type_t type)
{
    static const char *const reasons[] = {
        REASON_DRIVER_STARTED,
        REASON_ASSOCIATING,
        REASON_ASSOCIATED,
        REASON_ADDRESS_READY,
        REASON_ADDRESS_LOST,
        REASON_AUTHENTICATION_FAILED,
        REASON_ASSOCIATION_FAILED,
        REASON_DRIVER_FAILED,
        REASON_STOPPED,
    };
    return type <= WIFI_PLATFORM_EVENT_STOPPED ? reasons[type]
                                                : REASON_DRIVER_FAILED;
}
                    ...
                    const wifi_link_config_t *config)
{
    ...
    if (!is_wifi_link_config_valid(config)) {
        diagnostics_emit(DIAGNOSTIC_ERROR, COMPONENT_WIFI,
                         EVENT_CONFIG_INVALID, MESSAGE_CONFIG_INVALID);
        return false;
    }
    if (!is_wifi_link_config_enabled(config)) return true;
    ...
    if (!wifi_link->platform_initialized)
        diagnostics_emit(DIAGNOSTIC_ERROR, COMPONENT_WIFI,
                         EVENT_PLATFORM_INIT_FAILED,
                         MESSAGE_PLATFORM_INIT_FAILED);
...
static void enqueue_start_failure(wifi_link_t *wifi_link)
{
    ...
    diagnostics_emit(DIAGNOSTIC_ERROR, COMPONENT_WIFI,
                     EVENT_CONNECTION_ATTEMPT_FAILED,
                     MESSAGE_CONNECTION_ATTEMPT_FAILED);
}
...
{
    if (wifi_link->platform_initialized) platform_wifi_stop();
}
...
void wifi_link_process(wifi_link_t *wifi_link, uint64_t now_ms)
{
    ...
    for (size_t processed = 0; processed < MAXIMUM_EVENTS_PER_TICK;
         processed++) {
        if (!platform_wifi_get_event(&platform_event)) break;
        if (platform_event.type == WIFI_PLATFORM_EVENT_DRIVER_STARTED &&
        ...
        if (platform_event.type == WIFI_PLATFORM_EVENT_ASSOCIATED) {
            ...
            wifi_link->address_deadline_ms = now_ms + DHCP_TIMEOUT_MS;
                   ...
                   platform_event.type == WIFI_PLATFORM_EVENT_ADDRESS_LOST ||
                   event.type == NETWORK_EVENT_FAILED) {
            wifi_link->is_waiting_for_address = false;
        }
        const diagnostic_severity_t severity =
            event.type == NETWORK_EVENT_FAILED ||
                    event.type == NETWORK_EVENT_CONNECTION_LOST
                ? DIAGNOSTIC_WARNING
                : DIAGNOSTIC_INFO;
        ...
        if (platform_event.type == WIFI_PLATFORM_EVENT_ADDRESS_READY) {
            diagnostics_emit_limited(
                &wifi_link->event_rate_limiter, EVENT_RATE_WINDOW_MS,
                MAXIMUM_EVENTS_PER_WINDOW, severity, COMPONENT_WIFI,
                get_event_reason(platform_event.type), FORMAT_ADDRESS_READY,
            ...
            diagnostics_emit_limited(
                &wifi_link->event_rate_limiter, EVENT_RATE_WINDOW_MS,
                MAXIMUM_EVENTS_PER_WINDOW, severity, COMPONENT_WIFI,
                get_event_reason(platform_event.type), FORMAT_FAILURE_STATE,
                get_event_reason(platform_event.type),
                (unsigned)platform_event.reason_code,
            ...
            diagnostics_emit_limited(
                &wifi_link->event_rate_limiter, EVENT_RATE_WINDOW_MS,
                MAXIMUM_EVENTS_PER_WINDOW, severity, COMPONENT_WIFI,
                get_event_reason(platform_event.type), FORMAT_EVENT_STATE,
                get_event_reason(platform_event.type));
        }
        /* The neutral queue owns copied strings, so this stack event is safe. */
        (void)network_manager_enqueue_event(wifi_link->network_manager, &event);
    }
    if (wifi_link->is_waiting_for_address &&
        now_ms >= wifi_link->address_deadline_ms) {
        ...
        wifi_link->is_waiting_for_address = false;
        (void)network_manager_enqueue_event(wifi_link->network_manager,
                                            &timeout);
        diagnostics_emit(DIAGNOSTIC_WARNING, COMPONENT_WIFI,
                         REASON_DHCP_TIMEOUT, FORMAT_EVENT_STATE,
                         REASON_DHCP_TIMEOUT);
    }
}
...
void wifi_link_set_enabled(wifi_link_t *wifi_link, bool enabled,
                           uint64_t now_ms)
{
    network_manager_set_enabled(wifi_link->network_manager, NETWORK_LINK_WIFI,
                                enabled && wifi_link->platform_initialized,
                                now_ms);
}
...
{
    if (wifi_link->platform_initialized)
        network_manager_reconnect(wifi_link->network_manager,
                                  NETWORK_LINK_WIFI, now_ms);
}
