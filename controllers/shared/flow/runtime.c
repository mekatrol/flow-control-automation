...
static bool get_input_value(const flow_executable_t *flow, const bool values[FLOW_EXECUTABLE_MAX_NODES],
                            uint16_t node_index, const char *port_id, bool *value)
{
...
{
    if (runtime == NULL || runtime->executable == NULL || input == NULL ||
        (input->sample_count > 0U && input->samples == NULL))
    {
    ...
    for (uint16_t position = 0; position < flow->node_count; position++)
    {
        ...
        switch (node->kind)
        {
            ...
            case FLOW_NODE_MEMORY:
                ...
                break;
            case FLOW_NODE_NOT:
                if (!get_input_value(flow, working_values, node_index, "in", &left)) goto evaluation_failed;
                working_values[node_index] = !left;
                break;
            case FLOW_NODE_AND:
                if (!get_input_value(flow, working_values, node_index, "a", &left) ||
                    !get_input_value(flow, working_values, node_index, "b", &right)) goto evaluation_failed;
    ...
    for (uint16_t node_index = 0; node_index < flow->node_count; node_index++)
    {
        (void)snprintf(next.nodes[node_index].node_id, sizeof(next.nodes[node_index].node_id), "%s",
                       flow->nodes[node_index].id);
