...
static const char HEALTH_FORMAT[] =
    ...
    " rs485_errors=%" PRIu32 " rs485_queue_drops=%" PRIu32 " terminal=%s terminal_sessions=%" PRIu32
    " terminal_failed_logins=%" PRIu32 " terminal_output_drops=%" PRIu32;
...
int controller_health_format(char *output, size_t output_size, const controller_health_snapshot_t *snapshot)
{
    ...
    return snprintf(output, output_size, HEALTH_FORMAT, snapshot->uptime_ms, snapshot->free_heap_bytes, snapshot->wifi_state,
                    ...
                    snapshot->rs485_queue_drops, snapshot->terminal_state, snapshot->terminal_authenticated_sessions,
                    snapshot->terminal_failed_logins, snapshot->terminal_output_drops);
}
