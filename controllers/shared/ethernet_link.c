...
static const char *get_event_reason(ethernet_platform_event_type_t type)
{
    static const char *const reasons[] = {
        REASON_STARTED, REASON_LINK_UP, REASON_ADDRESS_READY,
        REASON_ADDRESS_LOST, REASON_LINK_DOWN, REASON_DRIVER_FAILED,
        REASON_STOPPED,
    };
    return type <= ETHERNET_PLATFORM_EVENT_STOPPED ? reasons[type]
                                                   : REASON_DRIVER_FAILED;
}

/* Initializes the W5500 adapter without waiting for link or DHCP. */
bool ethernet_link_init(ethernet_link_t *ethernet_link,
                        network_manager_t *network_manager,
                        const ethernet_link_config_t *config)
{
    *ethernet_link = (ethernet_link_t) {.network_manager = network_manager};
    if (!is_ethernet_link_config_valid(config) || !config->enabled) return false;
    ethernet_link->platform_initialized = platform_ethernet_initialize(config);
    if (!ethernet_link->platform_initialized)
        diagnostics_emit(DIAGNOSTIC_ERROR, COMPONENT_ETHERNET,
                         EVENT_INIT_FAILED, MESSAGE_INIT_FAILED);
...
void ethernet_link_start(ethernet_link_t *ethernet_link)
{
    if (!ethernet_link->platform_initialized || !platform_ethernet_start()) {
        ...
        diagnostics_emit(DIAGNOSTIC_ERROR, COMPONENT_ETHERNET,
                         EVENT_START_FAILED, MESSAGE_START_FAILED);
    }
}
...
{
    if (ethernet_link->platform_initialized) platform_ethernet_stop();
}
...
void ethernet_link_process(ethernet_link_t *ethernet_link)
{
    ...
    for (size_t processed = 0; processed < MAXIMUM_EVENTS_PER_TICK;
         processed++) {
        if (!platform_ethernet_get_event(&platform_event)) return;
        const network_event_t event = {
            .link_id = NETWORK_LINK_ETHERNET,
        ...
        if (platform_event.type == ETHERNET_PLATFORM_EVENT_ADDRESS_READY) {
            diagnostics_emit_limited(
                &ethernet_link->event_rate_limiter, EVENT_RATE_WINDOW_MS,
                MAXIMUM_EVENTS_PER_WINDOW, DIAGNOSTIC_INFO,
                COMPONENT_ETHERNET, REASON_ADDRESS_READY, FORMAT_ADDRESS,
            ...
            diagnostics_emit_limited(
                &ethernet_link->event_rate_limiter, EVENT_RATE_WINDOW_MS,
                MAXIMUM_EVENTS_PER_WINDOW, severity, COMPONENT_ETHERNET,
                get_event_reason(platform_event.type), FORMAT_STATE,
                get_event_reason(platform_event.type));
        }
        (void)network_manager_enqueue_event(ethernet_link->network_manager,
                                            &event);
    }
}
