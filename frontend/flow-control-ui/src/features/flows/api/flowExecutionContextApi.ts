...
const parse = (value: unknown): FlowExecutionContext => {
  ...
  if (
    ...
    !modes.includes(String(item.mode)) ||
    !states.includes(String(item.lifecycle)) ||
    !item.capabilities ||
  ...
  for (const name of [
    ...
    if (typeof capabilities[name] !== 'boolean')
      throw new Error('Execution context capabilities are malformed.');
...
export const flowExecutionContextApi = {
  create: (flowId: string, value: CreateExecutionContext) =>
    request(
