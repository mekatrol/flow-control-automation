...
namespace Tests.Unit.Points.Runtime;
...
public sealed class VirtualPointRuntimeStoreTests
{
    ...
    public async Task RestoresRetainedValuesAcrossRuntimeStoreRecreation()
    {
        ...
        var first = CreateStore(persistence);
        ...
        var restored = CreateStore(persistence);
    ...
    public async Task ClearsAndRestoresOnlyCompatibleInstanceQualifiedRetainedBackups()
    {
        var persistence = new RetainedStore();
        var contract = Analog("retained") with { Persistence = VirtualPointPersistenceType.Retained };
        var store = CreateStore(persistence);
        await store.ActivateFlowAsync("east", "writer", [contract], new HashSet<string> { contract.Key }, default);
    ...
    private static IVirtualPointRuntimeStore CreateStore(
        IVirtualPointRetainedStore? retainedStore = null)
    {
        var provider = TestServices.CreateProvider(services =>
        {
            if (retainedStore is not null)
            {
                services.Replace(ServiceDescriptor.Singleton(retainedStore));
            }
        });

        return provider.GetRequiredService<IVirtualPointRuntimeStore>();
    }
