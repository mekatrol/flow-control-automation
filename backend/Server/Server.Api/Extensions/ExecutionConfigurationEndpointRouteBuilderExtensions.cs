...
public static class ExecutionConfigurationEndpointRouteBuilderExtensions
{
    ...
    {
        endpoints.MapGet("/api/execution-configurations", async (IExecutionConfigurationService service, CancellationToken token) => Results.Json(await service.ListContextsAsync(token)));
        endpoints.MapGet("/api/execution-configurations/{id}", async (string id, IExecutionConfigurationService service, CancellationToken token) => await Map(() => service.GetContextAsync(id, token)));
        endpoints.MapPost("/api/execution-configurations", async (HttpRequest request, IExecutionConfigurationService service, IOptions<JsonOptions> options, CancellationToken token) => await DecodeAndMap<ExecutionContextDefinition>(request, options, value => service.SaveContextAsync(value, true, token), 201, token));
        endpoints.MapPut("/api/execution-configurations/{id}", async (string id, HttpRequest request, IExecutionConfigurationService service, IOptions<JsonOptions> options, CancellationToken token) => await DecodeAndMap<ExecutionContextDefinition>(request, options, value => EnsureId(id, value.Id, () => service.SaveContextAsync(value, false, token)), 200, token));
        endpoints.MapDelete("/api/execution-configurations/{id}", async (string id, IExecutionConfigurationService service, CancellationToken token) => await MapDelete(() => service.DeleteContextAsync(id, token)));
        ...
        endpoints.MapPut("/api/execution-configurations/{contextId}/deployments/{id}", async (string contextId, string id, HttpRequest request, IExecutionConfigurationService service, IOptions<JsonOptions> options, CancellationToken token) => await DecodeAndMap<ExecutionContextDeployment>(request, options, value => EnsureId(id, value.Id, () => EnsureId(contextId, value.ExecutionContextId, () => service.SaveDeploymentAsync(value, false, token))), 200, token));
        endpoints.MapDelete("/api/execution-configurations/{contextId}/deployments/{id}", async (string contextId, string id, IExecutionConfigurationService service, CancellationToken token) => await MapDelete(() => service.DeleteDeploymentAsync(contextId, id, token)));
